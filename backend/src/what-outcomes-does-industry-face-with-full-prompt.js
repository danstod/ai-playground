...
import { outcomesPrompt } from "./rag/prompts/value-story-prompt-outcomes.js";
...
const run = async () => {
  ...
  const outcomesQuestion = "List the desired outcomes";
  const challengesQuestion = "the main challenges or pain points";

  const outcomeDocs = await getDocs({
    ragQuestion: outcomesQuestion,
    vectorStore,
    k: 5,
  });
  const challengesDocs = await getDocs({
    ragQuestion: challengesQuestion,
    vectorStore,
    k: 5,
  });

  const docs = outcomeDocs.concat(challengesDocs);
  ...
  // Prompt the LLM
  const answer = await chain.invoke(outcomesPrompt);
