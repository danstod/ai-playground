...
export default class SentenceSplitterShortSentences {
  constructor(minLength = 30) {
    this.minLength = minLength;
  }

  splitText(text) {
    // Initial split based on sentence-ending punctuation
    ...
    while (i < sentences.length) {
      ...
      // If the sentence is shorter than the minimum length, combine it with the next one
      if (sentence.length < this.minLength && i < sentences.length - 1) {
        sentence += " " + sentences[i + 1].trim(); // Combine with the next sentence
        combinedSentences.push(sentence);
        i += 2; // Skip the next sentence as it's already combined
      } else {
        // Add the sentence as-is
        combinedSentences.push(sentence);
        i += 1;
      }
    }
...
// Example usage
// const text = "This is a short sentence. So is this one. But this sentence is quite a bit longer, providing more context. Here’s another short one. Indeed, we have several sentences now, and this should be a good test. The aim is to expand the use of the Cuvama app to the marketing, and BDRs teams and create a seamless customer journey focused on customer outcomes and this will improve Zellis’ customer retention and satisfaction, as well as increase revenue and reduce churn.";
const text = "Jedox is the world’s most adaptable planning and performance management platform. It has 50+ sales reps operating in four global regions. Over 2,500 organizations in 140 countries use Jedox to model scenarios, collate and analyze project data from any source, and simplify cross-organizational plans across all business systems.\n" +
    "\n" +
...
//   console.log(`Chunk ${index + 1}: ${sentence}`);
// });
